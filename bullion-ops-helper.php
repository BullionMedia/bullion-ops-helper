...
function bullion_ops_inject_muli_woff2() {
	...
	$weights = [
		...
		// only matching rule left is Elementor's TTF - the duplicate download
		// this is meant to stop. Add a real ExtraLight WOFF2 and remap if one
		// is ever wanted.
...
// --- Font + script perf trial, DEV ONLY (v0.9.58) --------------------------
//
// Three changes from the 2026-08-24 daily-health finding, gated to dev so live
// is untouched until the operator has looked. Remove the host gate to promote.
//
// Elementor Custom Fonts emits its Muli TTF @font-face set TWICE - 14
// declarations, weights 100-700, every one a .ttf. Our WOFF2 override covered
// only 300-700, and the homepage CSS asks for 100 and 200, so a TTF stayed the
// winning rule: Muli-Regular.ttf downloaded at 41.3 KB alongside its own
// 33.2 KB WOFF2. Same typeface, twice. §I trap 22 resurfacing.
//
// The existing override comment calls Elementor's TTF rules a "fallback for
